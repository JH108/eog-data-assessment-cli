...
// URL for the EOG data assessment
const REPO_URL = "https://github.com/jh108/eog-data-assessment-template";

class CreateCommand extends Command {
  async run() {
    const { flags } = this.parse(CreateCommand);
    const userName = flags.name || (await cli.prompt("What's Your Name?"));
    const directory = [parameterize(userName), "eog-data-assessment"].join("-");
    ...
    await simpleGit(directory).commit("eog-data initial commit");
    ...
    this.log(
      `\n${chalk.green(
        "SUCCESS"
      )}: Assessment created and in directory "${directory}"`
    );
    this.log(
      `\n${chalk.green(
        "SUCCESS"
      )}: You'll want to yarn install and then follow the prompts in index.js.`
    );
...
CreateCommand.description = `EOG Data Node.js Assessment
...
